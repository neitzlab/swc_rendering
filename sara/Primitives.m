...
% 7Sept2018 - SSP - updated namespace
	...
	methods (Static)
		...
		function [localCells, localPoints] = Line(varargin)
        	% LINE
        	%
        	% Inputs:
        	%	pt1 		[0, 0, 0] 	1st point at line start
        	%	pt2 		[0, 0, 0]	2nd point at line end
        	%	data 		[] 			Data attached to cell
        	%	pointStart 	0			Amount to shift start points by
        	% ---------------------------------------------------------
            ...
            ip = inputParser();
            ip.CaseSensitive = false;
            ip.addParameter('pt1', [0 0 0], @isnumeric);
            ip.addParameter('pt2', [0 0 0], @isnumeric);
            ip.addParameter('data', 0, @isnumeric);
            ip.addParameter('pointStart', 0, @isnumeric);
            ip.parse(varargin{:});
